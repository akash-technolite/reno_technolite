...
@WebServlet("/FinalReportUpdate")
public class FinalReportUpdate extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (formItems != null && formItems.size() > 0) {
                ...
                int report_id=Integer.parseInt(fieldvalue[0]);
                workReport.setReport_id(report_id);
               /* workReport.setTo(fieldvalue[1]);
                workReport.setContractorId(contractorId);
                workReport.setSubject(fieldvalue[2]);*/
                workReport.setMesage(fieldvalue[3]);
                /*workReport.setLeadId(Integer.parseInt(fieldvalue[4]));*/
                
                
                int res=contractorDao.updateFinalWorkReport(workReport);
                
                   if(res>0){
                	
                	   int resd=contractorDao.deleteFinalWorkReportFiles(report_id);   
                	   
                	   
                	   
                	EmailFormat emailformat= new EmailFormat();
                    
                    emailformat.setTo(fieldvalue[1]);
                    emailformat.setSubject(fieldvalue[2]);
                    emailformat.setMessage(fieldvalue[3]);
